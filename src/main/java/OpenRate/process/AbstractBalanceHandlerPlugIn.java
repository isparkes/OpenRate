...
/**
 * Now that we have the prioritised list of products and promotions, we can work
...
public abstract class AbstractBalanceHandlerPlugIn extends AbstractTransactionalPlugIn {
  ...
  // -----------------------------------------------------------------------------
  /**
   * Called when the underlying transaction is commanded to start. This should
   ...
   * close down all processing objects and go into a quiescent state. This
   * should return 0 if everything was OK, otherwise -1.
   *
  ...
  /**
   * Called when the underlying transaction is commanded to roll back, that
   * means to undo any data and finish.
   *
   * @param transactionNumber The number of the transaction
   */
  @Override
...
// -----------------------------------------------------------------------------
   ...
   * @param counterId The counter id to retrieve for
   * @param utcEventDate The date to retrieve for
   * @param initialValue The initial value of the counter in the case we create
   * it
   * @return The counter or null
   */
...
// -----------------------------------------------------------------------------
  /**
   * Consumes the given RUM value from a IRatingRecord record, creating a
   * balance if this is not found. The balance impacts are written directly back
   * into the record, and a discounting summary is passed back to the caller.
   *
   ...
   *
   * This method will consume up until exhaustion of the counter value,
   * splitting the input value if we are crossing a threshold.
   *
   * The discounting summary tells the user: - If the record was discounted - If
   * a balance impact was created - The ID and Record ID of the counter impacted
   * - The value consumed - The value of the counter after the impact
   *
   ...
   * marked with (*).
   *
   * @param currentRecord The record to be discounted, inherited from
   * IRatingRecord
  ...
  public DiscountInformation discountConsumeRUM(IRatingRecord currentRecord, String discountName, long balanceGroupId, String rumToUse, int counterId, double initialBalance, long utcBalanceStartValidity, long UTCBalanceEndValidity) {
    ...
    if (tmpCounter == null) {
      ...
      // If we create a counter, always report it back
   ...
   * balance impacts are written directly back into the record, and a
   * discounting summary is passed back to the caller.
   *
   ...
   * UTCEventDate
   *
   * @param currentRecord The record to be discounted, inherited fro
   * IRatingRecord
   * @param discountName The name of the discount
   * @param balanceGroupId The ID of the balance group
   * @param rumToUse The RUM to consume
   * @param counterId The ID of the counter to impact
   * @param initialBalance The initial balance the counter had, serves as a
   * maximum limit in refunds
   * @return The DiscountInformation summary object
   */
  ...
  /**
   * Aggregates the given RUM value from a IRatingRecord record, creating a
   * balance if this is not found. The balance impacts are written directly back
   * into the record, and a discounting summary is passed back to the caller.
   *
   ...
   * in this case, so the management of the counter is relatively
   * straightforward.
   *
   ...
   * The discounting summary tells the user: - If the record was discounted - If
   * a balance impact was created - The ID and Record ID of the counter impacted
   * - The value aggregated - The value of the counter after the impact
   *
   * In addition the following fields from the IRatingRecord are used: -
   * UTCEventDate
   *
